...
class HMC(object):
    ...
    def set_preferences_pcm(self):
        ...
        if r.status_code != 200:
            ...
            if r.status_code == 422:
                print("Error 422: 'The server understands the content type of the request & \nthe syntax is correct but it was unable to process the instructions'")
                print("Error 442 suggestions: Reboot the hmc or rise a PMR = Support Case with your support channel")
            if r.status_code == 401:
                print("Error 401: Authenication error (user/password) or your HMC is currently starting up")
            if self.debug:
    ...
    def get_filenames_energy(self, Atomid, Server):
        ...
        xxx = "Energy-" + Server
        if self.debug:
            print("DEBUG:get_filenames_energy(url=%s)" % (url))
        return self.get_filenames(url, xxx)
    ...
    def extract_ssp_info(self,stats):
        ...
        try:
            errId1         = data["sspUtil"]["utilSamples"][0]['sampleInfo']['errorInfo'][0]['errId']
            errMsg1        = data["sspUtil"]["utilSamples"][0]['sampleInfo']['errorInfo'][0]['errMsg']
        except:
            errId1         = 0
            errMsg1        = "none"
        ...
        ret = { 'ssp': sspName, 'cluster': clusterName, 'frequency': frequency, 'start': startTimeStamp, 'end': endTimeStamp, 'errId1': errId1, 'errMgs1': errMsg1 }
        return ret
    ...
    def extract_ssp_totals(self,stats):
        ...
        #self.debug = 1
        if self.debug:
            print("DEBUG:extract_ssp_totals(data)\n---\n%s"%(data))
            print("extract_ssp_totals: Size of data %d"%(len(data)))
            print("extract_ssp_totals: Size of Util %d"%(len(data["sspUtil"]["utilSamples"])))
            print("extract_ssp_totals: Util %d"%(len(data["sspUtil"]["utilSamples"][0])))
            print("extract_ssp_totals: Util data %s"%(data["sspUtil"]["utilSamples"][0]))
        #self.debug = 0
        ret = []
        bad=0
        good=0
        ...
        for sample in jdata['systemUtil']['utilSamples']:
            ...
            if status != 0 :
                ...
                try:
                    ...
                    print("Error Status non-zero but there is no error message . . . continue")

                print("*** Error Server %s: status=%d (not zero)\n**** mgs=%s" %(name,status,errmsg))
                print("**** errId=%s uuid=%s reportedBy=%s" %(errId,uuid,reportedBy))
                  ...
                  for v in sample['viosUtil']:
                    ...
                    if 'sriovLogicalPorts' in v['network']:
                        for n in v['network']['sriovLogicalPorts']:
                            ...
                            vios_net_reads  += n['receivedPackets'][0]
                            vios_net_writes += n['sentPackets'][0]
                    vios_proc_vp       += v['processor']['currentVirtualProcessors'][0]
    ...
    def extract_lpar_stats(self,data):
        ...
        for sample in jdata['systemUtil']['utilSamples']:
            ...
            if samplestatus != 0 :
                ...
                cpu_vp       = processor['currentVirtualProcessors'][0]
                cpu_entitled = processor['entitledProcUnits'][0]
                cpu_used     = processor['utilizedProcUnits'][0]
                # original cpu_used     = processor['utilizedUncappedProcUnits'][0] + processor['utilizedProcUnits'][0] + processor['utilizedCappedProcUnits'][0]
        ...
        for sample in data['systemUtil']['utilSamples']:
            if sample['sampleInfo']['status'] != 0 :
               ...
               errors+= 1
                ...
                count += 1
                headerline = 'time,watts,inlet'
                timeStamp = sample['sampleInfo']['timeStamp']
                resultline = { 'time':timeStamp[0:19] }

                thermal = sample['energyUtil']['thermalUtil']
                if 'energyUtil' in sample and 'powerUtil' in sample['energyUtil']:
                    try:
                        resultline.update( {'watts': sample['energyUtil']['powerUtil']['powerReading'][0]} )
                    except:
                        pass

                if 'cpuTemperatures' in thermal:
                    try: 
                        resultline.update( {'inlet': thermal['inletTemperatures'][0]['temperatureReading'][0]} )
                    except:
                        pass

                if 'baseboardTemperatures' in thermal:
                    for count,node in enumerate(thermal['baseboardTemperatures']):
                        try:
                            resultline.update( {'mb'+str(count): thermal['baseboardTemperatures'][count]['temperatureReading'][0]} )
                            headerline = headerline + ',mb'+str(count)
                        except:
                            pass

                if 'cpuTemperatures' in thermal:
                    for count,node in enumerate(thermal['cpuTemperatures']):
                        try:
                            resultline.update( {'cpu'+str(count): thermal['cpuTemperatures'][count]['temperatureReading'][0]} )
                            headerline = headerline + ',cpu'+str(count)
                        except:
                            pass
