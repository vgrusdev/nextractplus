...
class nchart_open(object):
    ...
    def output_top_unitstring(self, file, graphnum, unitstring):
        ...
        if self.debug:
            print("output_top_unitstring(graphnum=%d, unitstring=%s)"%(graphnum, unitstring))
        file.write('\tvar data_' + str(graphnum) +  ' = google.visualization.arrayToDataTable([\n')
    ...
    def output_end4(self, file, name, button1, button2, button3, button4):
        ...
        file.write('\t<button id="draw_4" style="color:orange;"><b>'+ button4 + '</b></button>\n')
    ...
    def nchart_energy(self, filename, info, data):
        ...
        self.output_bot(self.web,  1, info['server'] + " -  Electricity Watts")

        self.output_top(self.web, 2, 'Inlet Celius') 
        for row in data:
            self.web.write(",['Date(%s)', %d]\n" %(self.googledate(row['time']),row['inlet']))
        self.output_bot(self.web,  2, info['server'] + " - Room Celius")
        ...
        for key,value in data[0].items():
            if key[:2] == 'mb':
                ...
                count = count +1
        #print("planars=[%s]"%(headline))
        self.output_top_unitstring(self.web, 3, headline) 
        for row in data:
            self.web.write(",['Date(%s)'" %(self.googledate(row['time'])) )
            for key,value in row.items():
                if key[:2] == 'mb':
                    self.web.write(", %.1f" %(row[key]) )
            self.web.write("]\n" )
        self.output_bot(self.web, 3, info['server'] + ' - Planar Celsius')
        ...
        for row in data:
            ...
            for key,value in row.items():
                if key[:3] == 'cpu':
                    self.web.write(", %.1f" %(row[key]) )
            self.web.write("]\n" )
        self.output_bot(self.web, 4, info['server'] + ' - CPU Celius')

        self.output_end4(self.web, info['server'],'Watts','Inlet', 'Planar', 'CPU')
        self.web.close()
